...
echo "[$(echo ${GET_NODES}| jq ".items|length")] Nodes"
for i in $(echo ${GET_NODES}| jq ".items[]|[.metadata.name]|sort|.[]"|tr -d '"'); do
  echo "--$i"
done
...
for kind in $KINDS; do
  if [ $kind = "ReplicaSet" ]; then
    ITEMS=$(echo ${GET_ALL}|jq ".items[]|select(.kind == \"$kind\" and .status.replicas > 0)|[.metadata.name]|sort|.[]"|tr -d '"')
  else
    ITEMS=$(echo ${GET_ALL}|jq ".items[]|select(.kind == \"$kind\")|[.metadata.name]|sort|.[]"|tr -d '"')
  fi
  echo "$kind"
